...
simulated_annealing_Prochloro <- function(S,
  ...
  place <- which(F[,1:(ncol(F)-2)] > 0)
  
  if (is.null(user_defined_min_max)) {
    K <- Default_min_max(phytoclass::min_max, F[,(1:ncol(F)-2)], place)
    min.val <- K[[1]]
    max.val <- K[[2]]
  }
  else {
  ...
  condition.test <- Condition_test(S[,1:(ncol(S)-2)], F[,1:(ncol(F)-2)], min.val, max.val)
